...
import { Trash2, Plus, Briefcase, FileText, ExternalLink } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-10">
            <div className="container mx-auto px-4 max-w-6xl">
                ...
                {/* Tabs */}
                <div className="flex bg-white rounded-lg p-1 border border-gray-200 mb-8 w-max shadow-sm">
                    <button
                        onClick={() => setActiveTab('jobs')}
                        className={`px-6 py-2.5 rounded-md font-medium text-sm transition-colors flex items-center gap-2 ${activeTab === 'jobs' ? 'bg-primary text-white shadow-md' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}
                    >
                        <Briefcase size={18} /> Manage Jobs
                    </button>
                    <button
                        onClick={() => setActiveTab('applications')}
                        className={`px-6 py-2.5 rounded-md font-medium text-sm transition-colors flex items-center gap-2 ${activeTab === 'applications' ? 'bg-primary text-white shadow-md' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}
                    >
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <div className="p-6 border-b border-gray-200">
                            <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                                <FileText size={20} className="text-primary" /> Received Applications
                            </h2>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {loadingApps ? (
                                        <tr>
                                            <td colSpan="4" className="px-6 py-8 text-center text-gray-500">Loading applications...</td>
                                        </tr>
                                    ) : applications.length === 0 ? (
                                        <tr>
                                            <td colSpan="4" className="px-6 py-8 text-center text-gray-500">No applications received yet.</td>
                                        </tr>
                                    ) : (
                                        applications.map(app => (
                                            <tr key={app.id} className="hover:bg-gray-50">
                                                <td className="px-6 py-4">
                                                    ...
                                                    {app.cover_note && (
                                                        <p className="text-xs text-gray-500 mt-2 max-w-xs truncate" title={app.cover_note}>
                                                            "{app.cover_note}"
                                                        </p>
                                                    )}
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="font-medium text-gray-900 text-sm">{app.job_title}</div>
                                                    <div className="text-xs text-gray-500">{app.job_company}</div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <a
                                                        ...
                                                        className="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-blue-600 rounded-md text-sm font-medium hover:bg-blue-100 transition-colors"
                                                    >
